...
async def handle_razorpay_webhook(
    ...
    job = RecoveryOutbox(
        event_id=event.id,
        failure_class_hint=forced_failure_class.value if forced_failure_class else None,
        rationale_hint=forced_rationale,
        status=OutboxStatus.PENDING,
    )
    db.add(job)
    db.commit()
    # Keep the existing mock-mode UX immediate, but production returns after
    # durable intake and a separate worker performs external API work.
    if not settings.PROCESS_OUTBOX_INLINE:
        return {"status": "queued", "event_id": event.id, "outbox_id": job.id}
    action = process_outbox_job(db, job)
    return {
        "status": "processed",
        "event_id": event.id,
